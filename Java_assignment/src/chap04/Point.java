...
public class Point {
	...
	public void showAllPoints(List<Student> sList) { //학생 객체들이 들어있는 리스트가 와야죠?
		...
		for(int i=0;i<sList.size();i++) {
			...
			classAvgSum += sList.get(i).getAverage();
		}
		System.out.printf("- 반 평균: %.1f",classAvgSum/sList.size());
	...
	public void searchPoint(List<Student> students) {
		/*
		 ...
		 2. 찾는 학번이 존재하지 않는다면 검색하지 못했다는
		  메세지를 출력해 주세요.
		 */
		int idx = searchIdx(students);
		if(-1==idx) {
			System.out.println("유효하지 않은 학번입니다");
			return;
		}
		showPointUI();
		students.get(idx).outputStuInfo();
	...
	public void modifyPoint(List<Student> students) {
		...
		while(true) {
			...
			if(!flag) {
				students.get(idx).setKor(tempK);
				students.get(idx).setEng(tempE);
				students.get(idx).setMath(tempM);
				break;
			}
		}
		students.get(idx).calcTotAvgGrade();
	}
	...
	public void deletePoint(List<Student> students) {
		...
		System.out.println(students.get(idx).getName()+"님의 정보를 삭제합니다.[Y / N]");
	...
	public int searchIdx(List<Student> students) {
